...
**/
	...
	if (tmp == NULL) {
        //Vlozime uzol do prazdnehoo stromu
        (*RootPtr) = newNode;
	} else {
		//Kym nie je koniec
		while (tmp != NULL) {
			if (tmp->Cont == Content) {
				//Hodnota uz je v strome, uvolnime novy list
				free(newNode);
				return;
			} else
				if ( tmp->Cont > Content) {
					//Obsah je vacsi ako hodnota, lezi vlavo
					if (tmp->LPtr == NULL) {
						//vlavo nic neni, ulozi novy list
						tmp->LPtr = newNode;
                        return;
					} else {
					    //Vlavo nieco je, vlozi lavy
					    tmp = tmp->LPtr;
					}
				} else
				    if (tmp->Cont < Content) {
				        //Obsah je mensi ako hodnota, lezi vpravo
				        if (tmp->RPtr == NULL) {
				            //Vpravo nic neni, ulozi novy list
				            tmp->RPtr = newNode;
				            return;
				        } else {
				            //Vpravo nieco je, vlozi pravy
				            tmp = tmp->RPtr;
				        }
				    }
		}
...
**/
    //Prechadzame lavu vetvu az do konca
    while (ptr != NULL){
        //Zpracujeme uzly
        BTWorkOut(ptr);
        //Ulozime ukazatel do zasobnika
        SPushP(Stack, ptr);
        //Posunieme sa dolava
        ptr = ptr->LPtr;
    }
...
**/
    ...
    //Kym neni prazdny zasobnik
    while (!SEmptyP(&stack)){
        //vlozi spracovany uzol
        RootPtr = STopPopP(&stack);
        //prechadza prave uzly
...
**/
    ...
    while (!SEmptyP(&stack)) {
        //Prvy zo zasobnika, a vratime ho
        RootPtr = STopPopP(&stack);
        SPushP(&stack, RootPtr);

        //Narazili sme zas na uzol
        if (STopPopB(&stackB)) {
            //Prave uzly
            SPushB(&stackB, false);
            Leftmost_Postorder(RootPtr->RPtr, &stack, &stackB);
        } else {
            //Vyberiem uzol a spracujem ho
            RootPtr = STopPopP(&stack);
            BTWorkOut(RootPtr);
        }
    }
...
**/
    ...
    if ((*RootPtr) != NULL) {
        do {
            //Ked sme na konci ale zasobnik nie je prazdny
            if ((*RootPtr) == NULL && !(SEmptyP(&stack))) {
                *RootPtr = STopPopP(&stack);
            } else {
                if ((*RootPtr) != NULL) {
                    //Ulozi uzol na zasobnik
