...
*/
	...
	if (q == NULL) {
		queueError(QERR_INIT);					//Nevhodny parameter q
	} else {
		q->b_index = 0;							//Nastavi index prve volne pole na 0
		q->f_index = 0;							//Nastavi index prveho prvku na 0

		for (int i = 0; i < QUEUE_SIZE; i++) {
			q->arr[i] = '*';					//Vsetky hodnoty v poli nastavi na *
		}
	}
}
...
*/
	...
	return ((++index) % QUEUE_SIZE); 		//Najprv zvysi index potom vrati zvysok po deleni,
									 		// na konci sa posunie na zaciatok
}
...
*/
	...
	return (q->b_index == q->f_index); 		//prva volna pozice je zaroven prvy prvok
}
...
*/
	...
	return (nextIndex(q->b_index) == q->f_index); 		//Ked dalsi index volneho prvku sa rovna prvemu prvku
}
...
*/
	...
	if (queueEmpty(q)) {
		queueError(QERR_FRONT);			//Fronta je prazdna
	} else {
		*c = q->arr[q->f_index];		//Vlozi prvu hodnotu arr do c
	}
}
...
*/
	...
	if (queueEmpty(q)) {
		queueError(QERR_REMOVE);					//Fronta je prazdna
	} else {
		q->f_index = nextIndex(q->f_index);			//Druhy prvok sa stane prvym
	}
}
...
*/
	...
	if (queueEmpty(q)) {
		queueError(QERR_GET);			//Prazdna fronta
	} else {
		queueFront(q, c);				//Vlozi prvy znak fronty do c
		queueRemove(q);					//Odstrani prvy znak z fronty
	}
}
...
*/
	...
	if (queueFull(q)) {
		queueError(QERR_UP);						//Plna fronta
	} else {
		q->arr[q->b_index] = c;						//Najprv vlozi znak do volneho prvku
		q->b_index = nextIndex(q->b_index);			//Posunie index volneho prvku
	}
}
